    ...
    <section class="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="text-3xl font-semibold leading-tight sm:text-4xl">Create project</h1>
            </div>
            <a class="btn" href="{{ route('admin.projects.index') }}">Back</a>
        </div>

        <form class="card card-border mt-10 bg-base-100" method="POST" action="{{ route('admin.projects.store') }}">
            @csrf
